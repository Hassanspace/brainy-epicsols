...
import JobDetail from "./currentjob/JobDetail";
...
// Jobs data (moved here so Dashboard + CurrentJob + JobDetail can share it)
...
const Dashboard = () => {
  ...
  // Render logic for normal + dynamic tabs
  const renderTabContent = () => {
    // Friend detail
    if (activeTab.startsWith("Friend:") && selectedFriend) {
      return <FriendDetail friend={selectedFriend} />;
    }
    ...
    if (activeTab.startsWith("Job:")) {
      const jobId = activeTab.split(":")[1];
      const allJobs = [
        ...jobsData["Current Jobs"],
        ...jobsData["Upcoming Jobs"],
      ];
      const jobData = allJobs.find((j) => j.id === jobId);
      return <JobDetail job={jobData} />;
    }
    if (activeTab === "Current Job") {
      return (
        <CurrentJob
          onJobClick={(id) => setActiveTab(`Job:${id}`)}
          jobsData={jobsData}
        />
      );
    }
    ...
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <header className="flex items-center justify-between bg-white px-4 h-20 border-b border-[#DDDDDD] overflow-hidden">
        ...
        <div className="flex-1 flex justify-center md:justify-start items-center min-w-0">
          <img
            className="w-24 sm:w-28 md:w-32 max-w-full"
            src={logo}
            alt="Brainy Logo"
          />
        </div>
        ...
        <div className="hidden md:flex flex-shrink-0 justify-center">
          <div
            className="brainyai md:mr-25 mr-0 rounded-full flex items-center justify-center text-white text-sm sm:text-base lg:text-lg py-2 
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 min-w-0">
          ...
          <div className="flex justify-center items-center gap-1 h-8 sm:h-10 md:h-12 bg-[#F3F3FA] rounded-full px-2 sm:px-3 md:px-4 flex-shrink-0">
            <img
              ...
              alt="Profile"
            />
            <p className="text-xs sm:text-sm md:text-base truncate max-w-[60px] md:max-w-[100px]">
              Alison
            </p>
          </div>
        </div>
        ...
        >
              ...
              return (
                ...
                >
                  {Icon && (
                    <Icon
                      className={`w-5 h-5 ${
                        isActive ? "text-[#25D366]" : "text-black"
                      }`}
                    />
                  )}
                  <span>{tab}</span>
          ...
          <div className="p-4 ">
            <div className="relative max-w-3xl rounded-xl bg-gradient-to-br from-[#25D366] to-[#704FE6] p-[2px]">
              <img
                className="absolute top-[-6%] right-[-2%] w-14"
                src={rocket}
                alt=""
              />
